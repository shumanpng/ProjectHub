  ...
  def create
    ...
    respond_to do |format|
      if @task.save
        ...
        message = "Task #{@task.title} has been assigned to #{@assigned_user.name} by #{@current_user.name}"
        #@notification_targets = Group.where(:id => @task.group_id).group_memberships
